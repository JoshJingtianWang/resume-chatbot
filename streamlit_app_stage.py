...
st.sidebar.markdown("<h4 style='margin-top: -10px; color: gray;'>Powered by LangChain and gpt-3.5-turbo.</h4>", unsafe_allow_html=True)
...
- 🐱 [GitHub](https://github.com/joshjingtianwang)
""")
...
if "messages" not in st.session_state:
    first_message = 'What do you want to know about Josh?'
...
if user_input := (st.chat_input("You:", key="user_input") or st.session_state.get("button_question")):
    ...
    with st.spinner("Thinking..."):
        ...
        #st.write(f"condensed history: {preprocess_output['condensed_history']}")

        router_output = router_chain.invoke(preprocess_output)
        classification = router_output['classification'].datasources
        #router_progress_placeholder.write_stream(text_generator(f"Agent classified query as: {classification}"))
        router_progress_placeholder = display_progress_message(f"Agent classified query as: {classification}")
        st.session_state.progress_placeholders.append(router_progress_placeholder)

        retrieval_output = retrieval_chain.invoke(router_output)
        #retrieval_progress_placeholder.write_stream(text_generator("Retrieved relevant documents."))
        retrieval_progress_placeholder = display_progress_message("Retrieved relevant documents.")
        ...
        with st.session_state.assistant_response_placeholder.container():
            with st.chat_message("assistant"):
                ...
                if eval_result == "Y" and not summary_docs:
                    image_path = get_image_path_from_doc(retrieval_output['context'])
                    if image_path:
